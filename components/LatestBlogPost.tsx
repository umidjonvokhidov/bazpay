...
const LatestBlogPost = ({ blog }: { blog: LatestBlogPostProps }) => {
  return (
    <div className="flex gap-5 odd:flex-col max-lg:even:flex-col even:flex-col-reverse">
      <Image
        src={blog.image}
        alt={blog.title}
        width={0}
      ...
      <div className="border-grey-100 flex flex-col items-start gap-y-6 rounded-2xl border bg-white px-4 py-5">
        <div className="flex flex-col items-start gap-y-2">
          <h5 className='font-semibold'>{blog.date}</h5>
          <Link
            href={blog.path}
